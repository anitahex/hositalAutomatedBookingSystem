...
def build_page_seven() -> str:
    ...
    items = [
        ...
        "A reverse proxy on the host terminates HTTPS and forwards API and WebSocket traffic to the backend on loopback; the backend serves the frontend directly.",
    ]
    for item in items:
        block, y = draw_multiline(72, y, "- " + item, size=11, width=78, line_gap=17)
        parts.append(block)
